    ...
    <>
      ...
      <section className="bg-cream pb-10 pt-28 md:pt-32">
        <div className="flux-shell border-t border-ink/15 pt-5">
          ...
          <div className="grid gap-10 pb-16 pt-8 md:pb-20 md:pt-12 lg:grid-cols-[1.25fr_.75fr] lg:gap-16 lg:pb-24 lg:pt-16">
            <motion.div initial={{ opacity: 0, y: 36 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .75, ease: [0.16, 1, 0.3, 1] }}>
            ...
            <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .14, duration: .7, ease: [0.16, 1, 0.3, 1] }} className="border-t border-ink/15 pt-6 lg:self-end">
      ...
      <section className="overflow-hidden bg-white pb-24 pt-0 md:pb-36">
        ...
        <div className="flux-shell pt-10 md:pt-14">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .5 }} transition={{ duration: .6 }} className="mb-10 flex items-end justify-between border-t border-ink/15 pt-5"><p className="flux-kicker text-ink/45">System map</p><span className="text-xs text-ink/35">{String(content.cards.length).padStart(2, "0")} connected layers</span></motion.div>
      ...
      <section className="bg-ink py-24 text-white md:py-36">
        <div className="flux-shell">
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="flux-kicker mb-8 text-white/40">Connected knowledge</motion.p>
          <motion.h2 initial={{ opacity: 0, y: 36 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .4 }} transition={{ duration: .75, ease: [0.16, 1, 0.3, 1] }} className="max-w-4xl font-display text-display font-medium">Continue through the system.</motion.h2>
          <div className="mt-16 grid border-t border-white/20 md:grid-cols-2 lg:grid-cols-3">
            {content.related.map((card) => (
              <Link key={card.title} href={card.href ?? "/business-intelligence-audit"} className="group min-h-[260px] border-b border-white/20 p-7 transition-[background,color,padding] duration-500 hover:bg-white hover:text-ink md:border-r md:p-9 md:hover:px-11">
